...
takes the low-rank matrix $D_r$, and enforces low-entropy via MRF.  It
is worth noting that the first term in the second problem is a unary
term ensuring the consistency between $D_r$ and $D_m$, the second term
is a simple Potts model penalizing the label changes, and the third term
is the label cost~\cite{Delong2012} further enforcing the low-entropy
structure.
%
The iteration converges very quickly in general (2 to 3 iterations), and
the final offset-map is set to $D_m$, which requires few vertices and
faces (See Fig.~\ref{fig:comp_details}).
...
% structure.
$\mu_1$ is $\sqrt{\max(W, H)}$ as in ~\cite{Candes2011}, where $W$ and
$H$ are the dimension of the offset-map in pxiels. $\mu_2$ and $\mu_3$
are set to $500$ and $10^5$, respectively.
